...
$klein->respond('GET', '/random', function($request, $response, $service, $app) {
    ...
    $supported = ['1.9','1.8.9','1.8','1.7.10'];
    $random = array();
    $version_redirect = '';
 ...
 * @return redirect
 */
$klein->respond('GET', '/versions/[*:version]', function($request, $response, $service, $app) {
    $response->redirect('/version/' . $request->param('version'), 301);
});
...
$klein->respond('GET', '/submit/[form|failed|success|incomplete:state]', function ($request, $response, $service, $app) {
    ...
    $blacklist = array('66.58.219.148','98.216.149.30');
